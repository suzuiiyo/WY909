...
void outputMessage(QtMsgType type, const QMessageLogContext &context, const QString &msg)
{
    ...
    else
    {
        ...
        int fileCount = fileInfo.count();			//文件个数

        if(fileCount < 6500){
            QFile logFile(logPath + QString("暂态比对日志_") + QString("%1").arg(QDateTime::currentDateTime().toString("yy-MM-dd hh:mm:ss") + QString(".log")));
            ...
            int i= 0;
            do{
                QFileInfo infof = fileInfo.at(i);
                if(infof.fileName()=="." || infof.fileName()==".."){
                    i++;
                    continue;
                }else{
                    //清理180天之前的日志
                    QDateTime delDateTime = QDateTime::currentDateTime().addDays(-180);
                    qint64 nSecs = delDateTime.secsTo(infof.created());
                    if(nSecs < 0)
                    {
                        infof.dir().remove(infof.fileName());
                    }
                }
                i++;
...
{
    DBTransientSwitchConclusionRecordDAO* ttsr = new DBTransientSwitchConclusionRecordDAO();
    ttsr->doCreateTable();
    ...
    QFile paraFile(logPath + QString("参数暂存.log"));
    if(paraFile.open(QIODevice::ReadWrite | QIODevice::Append))
    {
        QTextStream text_stream(&paraFile);
        for(int i=1; i< argc; i++){
            text_stream << argv[i];
            text_stream << " ";
        }
    ...
    //判断文件是否存在
    bool fileStatus = recComFile.getFileExist();
    //传递四元组
...
//            bool fileExist = recComFile.getFileExist();
//            if(fileExist){
//               bool transSucc = recComFile.transCompareFile();
//               //写入保护比对结论
//               if(transSucc){
//                    QString protFileName1 = protPath1.split("/").last();
//                    QString protFileName2 = protPath2.split("/").last();
//                    //以本次比对保护文件名为索引查库
//                    QString indexFileName = argv[2];
//                    QString thisFileName = indexFileName.split("/").last();
//                    //对套保护文件名
//                    QString comFileNameString = nullptr;
//                    if(!QString::compare(thisFileName, protFileName1))
//                    {
//                        comFileNameString.append(protFileName2);
//                    }else
//                    {
//                        comFileNameString.append(protFileName1);
//                    }

//                    DBOrmOperation::List indexList;
//                    DBOrmOperation::Ptr dbo(new DBOrmOperation());
//                    dbo->setDataId(TransientTaskConclusionRecordORM::standardFileName);
//                    dbo->setDataType(DBOrmOperation::Char);
//                    dbo->setOpType(DBOrmOperation::OpType::seq);
//                    //以对套保护文件名查询
//                    dbo->setValue(comFileNameString);
//                    indexList.append(dbo);

//                    TransientTaskConclusionRecord::List conclusionListProtCompare = ttcr->doQuery(indexList);
//                    QStringList taskFileNameList;
//                    foreach(QSharedPointer<TransientTaskConclusionRecord> var, conclusionListProtCompare){
//                        QString taskFileNameVar = var->taskFileName();
//                        taskFileNameList.append(taskFileNameVar);
//                    }

//                    //如果查出来的不包含本套保护文件名
//                    if(!taskFileNameList.contains(thisFileName))
//                    {
//                        ResultIntoDb* resultDb = recComFile.getResultIntoDb();
//                        //计算保护比对结果
//                        protInsertStatus = resultDb->productProtResult();
