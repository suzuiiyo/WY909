...
public:
    ...
    QMap<QStringList, QList<int>> getKeySwicthMap() const;
    ...
    //获取比对值序列集
    QList<QList<QVector<double>>> getCompareDatalistList() const;

    //获取比对点序号序列集
    QList<QList<QList<int>>> getPointSeqlistList() const;
    //获取比对结论
    QList<double> getCompareConclusionList() const;
    //获取比对挑值序列集
    QList<QList<QVector<float>>> getInsertCompareDataList() const;
    //获取突变点时刻
    ...
    //获取开关量比对关键参数
    bool parseSwitchParameter(const QString&, const QString &, QList<int>);
    ...
    double getResetTime1() const;
    double getResetTime2() const;

    QList<QVector<double>> getRelativeErrorVectorList() const;

    QList<QVector<double>> getAbsoluteErrorVectorList() const;

    QList<QList<int>> getPointConclusionVectorList() const;

    //开关量参数
    QStringList getDcChDescribeList() const;

    QStringList getDcWordList() const;

    QList<int> getLackNumList() const;

    QList<QList<int>> getDcConclusionlistList() const;

    //保护动作
    QList<QList<int>> getDcCompareResultlistList() const;
//    断路器动作
    QList<QStringList> getJumpAckResultlistList() const;
    QList<QList<int>> getDcDelaylistList() const;

    //采样段第一个点的序号
    QList<int> getFirstPosList() const;

    //取采集第一个有效值的序号
    int getRecordFirPos(int, int) const;
...
private:
    ...
    //第2项保存的第三次动作时间差
    //依此类推
    QList<QList<int>> swComparelistList;
    ...
    //开关量跳变时间差
    QList<QList<int>> swJumpSpanlistList;

    //开关量动作结论
    QList<QStringList> swJumpAckResultlistList;
    ...
    //通道总数
    QList<int> countList1;
    QList<int> countList2;

    //比对结论
    QList<double> compareConclusionList;
    QList<int> compareResultList;
    ...
    //比对挑点值序列
    QList<QList<QVector<float>>> insertCompareDataList;
    //比对值序列集
    QList<QList<QVector<double>>> compareDatalistList;
    //比对点序号序列集
    QList<QList<QList<int>>> pointSeqlistList;
    //相对误差
