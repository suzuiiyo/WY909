...
#include <QVariant>
#include <QDateTime>
DBRecvOptPowerDAO::DBRecvOptPowerDAO()
    : m_connectionName("connectRecvOptPower")
{
...
int DBRecvOptPowerDAO::countRecord(const QString &iedName, bool *ok)
{
    bool bSucc = false;
    int count = 0;
    ...
    {
        QSqlQuery query(db);
        QString sql = QObject::tr("SELECT * FROM recvoptpower WHERE IEDNAME='%1'")
                      .arg(iedName);
...
RecvOptPower::List DBRecvOptPowerDAO::doQuery(SearchCondition::List &lst, bool *ok)
{
    bool bSucc = false;
    RecvOptPower::List lst_act;
    ...
    if (db.isOpen() || db.open())
    {
        ...
        for (iter = lst.begin(); iter != lst.end(); iter++)
        {
            ...
            sql = QObject::tr("SELECT * FROM recvoptpower WHERE IEDNAME='%1' ORDER BY GUID DESC LIMT %2,%3")
                    .arg(iedName)
                    .arg(startCount)
                    .arg(recordCount);
            bSucc = query.exec(sql);
            if (bSucc)
            {
                while (query.next())
                {
                    RecvOptPower::Ptr ptr_pv(new RecvOptPower());
...
RecvOptPower::List DBRecvOptPowerDAO::doQuery(const QString &dataRef, qint64 startTime, qint64 endTime, bool *ok)
{
    bool bSucc = false;
    RecvOptPower::List lst;
    ...
    {
        QSqlQuery query(db);
        QString sql = QObject::tr("SELECT * FROM recvoptpower WHERE DATAREF='%1' AND unix_timestamp(RECORDTIME)*1000>=%2 AND unix_timestamp(RECORDTIME)*1000<=%3 ORDER BY GUID ASC")
                        .arg(dataRef)
                        .arg(startTime)
                        .arg(endTime);
        bSucc = query.exec(sql);
        if (bSucc)
        {
            while (query.next())
            {
                RecvOptPower::Ptr ptr(new RecvOptPower());
...
bool DBRecvOptPowerDAO::doInsert(RecvOptPower::List &lst)
{
    bool bSucc = false;
    if (lst.size() == 0)
        return bSucc;
    ...
    if (db.isOpen() || db.open())
    {
        ...
        QString sql = "";
        RecvOptPower::List::iterator iter;
        int count = 0;
        for (iter = lst.begin(); iter != lst.end(); iter++)
        {
            RecvOptPower::Ptr ptr(new RecvOptPower());
            ptr = *iter;
            sql = QObject::tr("INSERT INTO recvoptpower VALUES(NULL, '%1', '%2', '%3', %4, '%5', '%6', '%7', '')")
                    .arg(ptr->iedName())     // 1
                    .arg(ptr->dataRef())     // 2
                    .arg(ptr->dataDesc())    // 3
                    .arg(ptr->dataType())    // 4
...
bool DBRecvOptPowerDAO::doInsert(QSqlDatabase db, RecvOptPower::List &lst)
{
    bool bSucc = false;
    if (lst.size() == 0)
      return bSucc;
    ...
    for (iter = lst.begin(); iter != lst.end(); iter++)
    {
        RecvOptPower::Ptr ptr(new RecvOptPower());
        ptr = *iter;
        sql = QObject::tr("INSERT INTO recvoptpower(GUID, IEDNAME, DATAREF, DATADESC, DATATYPE, DATAVALUE, QUALITY, RECORDTIME, SCOPE) VALUES(NULL, '%1', '%2', '%3', %4, '%5', '%6', '%7', '')")
                    .arg(ptr->iedName())     // 1
                    .arg(ptr->dataRef())     // 2
                    .arg(ptr->dataDesc())    // 3
                    .arg(ptr->dataType())    // 4
...
bool DBRecvOptPowerDAO::doCreateTable()
{
    bool bSucc = false;
    QSqlDatabase db = DBConnectionReal::connection(m_connectionName, "QMYSQL", "mmsdb");
    if(db.isOpen() || db.open())
    {
        ...
        QSqlQuery query(db);
        QString sql = QObject::tr("CREATE TABLE IF NOT EXISTS recvoptpower("
