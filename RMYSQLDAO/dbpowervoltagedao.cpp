...
#include <QVariant>
#include <QDateTime>
DBPowerVoltageDAO::DBPowerVoltageDAO()
    : m_connectionName("connectPowerVoltage")
{
...
int DBPowerVoltageDAO::countRecord(const QString &iedName, bool *ok)
{
    bool bSucc = false;
    int count = 0;
    ...
    {
        QSqlQuery query(db);
        QString sql = QObject::tr("SELECT * FROM powervoltage WHERE IEDNAME='%1'")
                        .arg(iedName);
...
PowerVoltage::List DBPowerVoltageDAO::doQuery(SearchCondition::List &lst, bool *ok)
{
    bool bSucc = false;
    PowerVoltage::List lst_act;
    ...
    if (db.isOpen() || db.open())
    {
        ...
        for (iter = lst.begin(); iter != lst.end(); iter++)
        {
            ...
            sql = QObject::tr("SELECT * FROM powervoltage WHERE IEDNAME='%1' ORDER BY GUID DESC LIMT %2,%3")
                    .arg(iedName)
                    .arg(startCount)
                    .arg(recordCount);
            bSucc = query.exec(sql);
            if (bSucc)
            {
                while (query.next())
                {
                    PowerVoltage::Ptr ptr_pv(new PowerVoltage());
...
PowerVoltage::List DBPowerVoltageDAO::doQuery(const QString &dataRef, qint64 startTime, qint64 endTime, bool *ok)
{
    bool bSucc = false;
    PowerVoltage::List lst;
    ...
    {
        QSqlQuery query(db);
        QString sql = QObject::tr("SELECT * FROM powervoltage WHERE DATAREF='%1' AND unix_timestamp(RECORDTIME)*1000>=%2 AND unix_timestamp(RECORDTIME)*1000<=%3 ORDER BY GUID ASC")
                        .arg(dataRef)
                        .arg(startTime)
                        .arg(endTime);
        bSucc = query.exec(sql);
        if (bSucc)
        {
            while (query.next())
            {
                PowerVoltage::Ptr ptr(new PowerVoltage());
...
bool DBPowerVoltageDAO::doInsert(PowerVoltage::List &lst)
{
    bool bSucc = false;
    if (lst.size() == 0)
        return bSucc;
    ...
    if (db.isOpen() || db.open())
    {
        ...
        QString sql = "";
        PowerVoltage::List::iterator iter;
        int count = 0;
        for (iter = lst.begin(); iter != lst.end(); iter++)
        {
            PowerVoltage::Ptr ptr(new PowerVoltage());
            ptr = *iter;
            sql = QObject::tr("INSERT INTO powervoltage VALUES(NULL, '%1', '%2', '%3', %4, '%5', '%6', '%7', '')")
                    .arg(ptr->iedName())     // 1
                    .arg(ptr->dataRef())     // 2
                    .arg(ptr->dataDesc())   //3
                    .arg(ptr->dataType())   // 4
...
bool DBPowerVoltageDAO::doInsert(QSqlDatabase db, PowerVoltage::List &lst)
{
    bool bSucc = false;
    if (lst.size() == 0)
        return bSucc;
    ...
    for (iter = lst.begin(); iter != lst.end(); iter++)
    {
        PowerVoltage::Ptr ptr(new PowerVoltage());
        ptr = *iter;
        sql = QObject::tr("INSERT INTO powervoltage(GUID, IEDNAME, DATAREF, DATADESC, DATATYPE, DATAVALUE, QUALITY, RECORDTIME, SCOPE) VALUES(NULL, '%1', '%2', '%3', %4, '%5', '%6', '%7', '')")
                .arg(ptr->iedName())     // 1
                .arg(ptr->dataRef())     // 2
                .arg(ptr->dataDesc())   //3
                .arg(ptr->dataType())   // 4
...
bool DBPowerVoltageDAO::doCreateTable()
{
    bool bSucc = false;
    QSqlDatabase db = DBConnectionReal::connection(m_connectionName, "QMYSQL", "mmsdb");
    if(db.isOpen() || db.open())
    {
        ...
        QSqlQuery query(db);
        QString sql = QObject::tr("CREATE TABLE IF NOT EXISTS powervoltage("
