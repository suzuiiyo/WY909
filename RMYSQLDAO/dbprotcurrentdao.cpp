...
#include "dbprotcurrentdao.h"
DBProtCurrentDAO::DBProtCurrentDAO()
    : m_connectionName("connectProtCurrent")
{
...
bool DBProtCurrentDAO::doInsert(QSqlDatabase db, ProtCurrent::List &lst)
{
    bool bSucc = false;
    if (lst.size() == 0)
      return bSucc;
    ...
    QString sql = "";
    ProtCurrent::List::iterator iter;
    int count = 0;
    for (iter = lst.begin(); iter != lst.end(); iter++)
    {
        ProtCurrent::Ptr ptr(new ProtCurrent());
        ptr = *iter;
        sql = QObject::tr("INSERT INTO protcurrent(GUID, IEDNAME, DATAREF, DATADESC, DATATYPE1, DATAVALUE1, DATATYPE2, DATAVALUE2, QUALITY, RECORDTIME) VALUES(NULL, '%1', '%2', '%3', %4, '%5', %6, '%7', '%8', '%9', '')")
                .arg(ptr->iedName())     // 1
                .arg(ptr->dataRef())     // 2
                .arg(ptr->dataDesc())    // 3
                .arg(ptr->dataType1())   // 4
...
bool DBProtCurrentDAO::doCreateTable()
{
    bool bSucc = false;
    ...
    if(db.isOpen() || db.open())
    {
        ...
        QSqlQuery query(db);
        QString sql = QObject::tr("CREATE TABLE IF NOT EXISTS protcurrent("
