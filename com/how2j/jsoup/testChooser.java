...
public class testChooser {
    ...
    private static void show(String text, String selector, int limit) {
        ...
        if (es.size() > 1) {
            System.out.println(String.format("%s - using selector is:\"%s\" \t (max show %d kill)", text, selector, limit));
        } else {
            System.out.println(String.format("%s - using selector is:\"%s\"", text, selector));
        }
        int i = 0;
        for (Element e : es) {
